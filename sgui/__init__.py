...
class textareaInfo:
    def __init__(self, w=100,h=100,value="", selected=False, cursor=0, selection_start=0,resizing=False):
        self.value = value
        self.selected = selected
        self.cursor = cursor
        self.selection_start = selection_start
...
class textarea(Widget):
    def __init__(self,w:int,h:int,label:str,id:str,default="") -> textareaInfo:
        bg = _rl.Color(128, 128, 128, 64)
        fg = _rl.Color(255, 255, 255, 255)
        if f"ta_{id}" in GUIState.Widgets:
            self.obj = GUIState.Widgets[f"ta_{id}"]
        else:
            self.obj = TextInput(default)
            GUIState.Widgets[f"ta_{id}"] = self.obj
        self.selected = self.obj.selected
        self.cursor = self.obj.cursor
        self.value = self.obj.value
        self.selection_start = self.obj.selection_start
        ...
        if super().__init__(winx,winy,w,h):
            self = None
            return
        self.w = max(self.w, _measure_text(self.value.split('\n')[0]+" ",10)+5)
        if not curwindow.h < self.y+5:
            if _rl.check_collision_point_rec(self.mp, [self.x+self.off.x, self.y+self.off.y, self.w, self.vh]):
                bg.a = 128
                if _rl.is_mouse_button_down(_rl.MouseButton.MOUSE_BUTTON_LEFT):
                    fg = _rl.Color(102, 191, 255, 255)
                    click_x = self.mp.x - (self.x + self.off.x)
                    click_y = self.mp.y - (self.y + self.off.y)
                    lines = self.value.split('\n')
                    line_num = min(len(lines)-1, max(0, int(click_y / 12)))
                    char_pos = min(len(lines[line_num]), max(0, int(click_x / 6)))
                    self.cursor = sum(len(line) + 1 for line in lines[:line_num]) + char_pos
                    if not _rl.is_key_down(_rl.KeyboardKey.KEY_LEFT_SHIFT):
                        self.selection_start = self.cursor
                if _rl.is_mouse_button_released(_rl.MouseButton.MOUSE_BUTTON_LEFT):
                    self.selected = True
        ...
        if self.selected:
            GUIState.Keyboard += (string.ascii_letters + string.digits + string.punctuation).split()
            GUIState.Keyboard.append("Backspace")
            GUIState.Keyboard.append("Return")
            charint = _rl.get_char_pressed()
            charstr = chr(charint)
            if _rl.is_key_pressed(_rl.KeyboardKey.KEY_ENTER):
                charint = 1
                charstr = "\n"
            ...
            if charint != 0:
                if self.selection_start != self.cursor:
                    ...
                    self.value = self.value[:self.cursor] + charstr + self.value[self.cursor:]
                    self.cursor += 1
                self.selection_start = self.cursor
        ...
        if self.value == "":
            ...
            for line_num, line in enumerate(lines):
                ...
                if self.selection_start != self.cursor:
                    sel_start = min(self.selection_start, self.cursor)
                    sel_end = max(self.selection_start, self.cursor)
                    
                    if line_start <= sel_end and line_end >= sel_start:
                        pre_start = line_start
                        pre_end = max(line_start, min(line_end, sel_start))
                        sel_start_line = max(line_start, min(line_end, sel_start))
                        sel_end_line = max(line_start, min(line_end, sel_end))
                        post_start = min(line_end, max(line_start, sel_end))
                        post_end = line_end
                        
                        pre_text = line[pre_start-line_start:pre_end-line_start]
                        sel_text = line[sel_start_line-line_start:sel_end_line-line_start]
                        post_text = line[post_start-line_start:post_end-line_start]
                        
                        pre_width = _measure_text(pre_text,10)
                        sel_width = _measure_text(sel_text,10)
                        
                        _rl.draw_text(pre_text,self.x+2,self.y+1+y_offset,10,fg)
                        _rl.draw_rectangle(self.x+4+pre_width,self.y+1+y_offset,sel_width,12,_rl.Color(20,128,255,200))
                        _rl.draw_text(sel_text,self.x+3+pre_width,self.y+1+y_offset,10,fg)
                        _rl.draw_text(post_text,self.x+4+pre_width+sel_width,self.y+1+y_offset,10,fg)
                    else:
                        _rl.draw_text(line,self.x+2,self.y+1+y_offset,10,fg)
                else:
                    _rl.draw_text(line,self.x+2,self.y+1+y_offset,10,fg)
                    if self.selected and round(_rl.get_time())%2 == 0:
                        if line_start <= self.cursor <= line_end:
                            cursor_x = self.x+2+_measure_text(line[:self.cursor-line_start],10)
                            _rl.draw_text("|",cursor_x,self.y+1+y_offset,10,fg)
                y_offset += 12
        ...
        GUIState.Widgets[f"ta_{id}"].value = self.value
        GUIState.Widgets[f"ta_{id}"].selected = self.selected
        GUIState.Widgets[f"ta_{id}"].cursor = self.cursor
        GUIState.Widgets[f"ta_{id}"].selection_start = self.selection_start
        _log(self.value.encode("utf-8"))
